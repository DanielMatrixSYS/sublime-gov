...
function panel:Sort(sort, desc)
    ...
    for i = 1, #everyone do
        local player    = everyone[i];
        local license   = player:getDarkRPVar("HasGunlicense") == true and true or false;

        table.insert(players, {player = player, nick = player:Nick(), job = player:Team(), HasLicense = license});
    end

    if (sort == "name") then
        table.sort(players, function(a, b) return a.nick < b.nick end);
    elseif(sort == "occupation") then
        table.sort(players, function(a, b) return a.job < b.job end);
    else
        table.sort(players, function(a, b) return a.HasLicense and not b.HasLicense end);
    end
...
function panel:PostInit(clear)
    ...
    self:AddColumn(self.L("ui_license_has_license"), columnWidth - 3, true, "license");
...
function panel:AddColumn(name, size, last, sort_type, func)
    ...
    column.DoClick = function()
        column.Descending = !column.Descending;

        self:Sort(sort_type, column.Descending);
    end
...
function panel:AddRow(player, pos)
    ...
    row.Avatar:SetPlayer(player, 184);

    row.PerformLayout = function(s, w, h)
        s.Avatar:SetPos(7, (h / 2) - 8);
        s.Avatar:SetSize(16, 16);
    end

    local pNick     = player:Nick();
    local pTeam     = team.GetName(player:Team());
    local pLicense  = player:getDarkRPVar("HasGunlicense");
    local lColor    = pLicense == true and SublimeUI.Green or SublimeUI.Red;
    local tColor    = team.GetColor(player:Team());
    ...
    row.Paint = function(s, w, h)
        ...
        SublimeUI.DrawTextOutlined(player:Nick(), "SublimeUI.16", 30, (h / 2) - 9, SublimeUI.White, SublimeUI.Black, TEXT_ALIGN_LEFT);
        SublimeUI.DrawTextOutlined(team.GetName(player:Team()), "SublimeUI.16", pos, (h / 2) - 9, tColor, SublimeUI.Black, TEXT_ALIGN_LEFT);
        SublimeUI.DrawTextOutlined(pLicense and self.L("ui_license_yes") or self.L("ui_license_no"), "SublimeUI.16", pos * 2, (h / 2) - 9, lColor, SublimeUI.Black, TEXT_ALIGN_LEFT);
    end
...
function panel:Paint(w, h)
    ...
    SublimeUI.DrawTextOutlined(self.L("ui_licenses"), "SublimeUI.16", 7, 8, SublimeUI.White, SublimeUI.Black, TEXT_ALIGN_LEFT);
end
vgui.Register("SublimeGov.Connected.Apps.Licenses", panel, "EditablePanel");

net.Receive("SublimeGov.AdjustedGunLicenseSuccess", function()
